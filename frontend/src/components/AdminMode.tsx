...
const AdminMode = () => {
  const { executePurge, triggerBotAttack, loading, error } = useSentinelStore();
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-3 font-mono">
      ...
      <div className="bg-black/90 border border-white/10 p-4 rounded-xl backdrop-blur-xl shadow-2xl w-72">
        ...
        <div className="grid grid-cols-1 gap-2">
          <button 
            ...
            className="flex items-center justify-between px-3 py-2 bg-orange-500/5 border border-orange-500/20 text-orange-500 rounded text-[10px] uppercase font-bold hover:bg-orange-500 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {actionLoading === "bot" ? (
      ...
      <div className="bg-black/90 border border-white/10 p-4 rounded-xl backdrop-blur-xl shadow-2xl w-72">
        ...
        <input 
          ...
          className="w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-xs text-cyan-400 mb-3 focus:outline-none focus:border-cyan-500/50 disabled:opacity-50 disabled:cursor-not-allowed"
        />
        <button 
          onClick={handlePurge}
          disabled={actionLoading === "purge" || loading}
          className="w-full flex items-center justify-center gap-2 py-2 bg-red-500/10 border border-red-500/20 text-red-500 rounded text-[10px] uppercase font-bold hover:bg-red-500 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {actionLoading === "purge" ? (
