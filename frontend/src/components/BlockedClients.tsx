...
import { ShieldAlert, Globe, Clock, Loader2 } from "lucide-react";

const BlockedClients = () => {
  const { blockedClients, loading, error } = useSentinelStore();
  ...
  return (
    <div className="w-full h-full flex flex-col font-mono">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`text-[10px] ${loading ? 'text-yellow-500/50' : 'text-cyan-500/50'}`}>
          {loading ? 'SYNCING...' : 'LIVE_SYNC_ACTIVE'}
        </span>
      </div>

      {error && (
        <div className="mb-4 p-2 bg-red-500/20 border border-red-500/30 rounded text-[10px] text-red-500">
          {error}
        </div>
      )}

      <div className="flex-1 overflow-auto custom-scrollbar">
        {loading && !blockedClients.length ? (
          <div className="h-full flex items-center justify-center text-white/40">
            <Loader2 className="w-4 h-4 animate-spin" />
            <span className="ml-2 text-xs">Loading blocked clients...</span>
          </div>
        ) : (
        ...
        {!loading && blockedClients.length === 0 && (
